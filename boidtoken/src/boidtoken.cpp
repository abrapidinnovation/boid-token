    ...
    uint32_t time_limit)
{
    ...
    microseconds expiration_time = time_limit == 0 ? microseconds(0) : t.time_since_epoch() + microseconds(time_limit * MICROSEC_MULT);
...
ACTION boidtoken::stakebreak(uint8_t on_switch)
{
    ...
    bool use_staked_balance)
{
    ...
    microseconds curr_time = t.time_since_epoch(),
                 expiration_time = time_limit == 0 ? microseconds(0) : curr_time + microseconds(time_limit * MICROSEC_MULT),
    ...
    bool transfer)
{
    ...
    if ((c_itr->stakebreak == STAKE_BREAK_ON || to_staked_balance) && !issuer_unstake && !transfer)
    {
        require_auth(from);
    ...
    else if ((c_itr->stakebreak == STAKE_BREAK_ON || to_staked_balance) && !issuer_unstake && transfer)
    {
        require_auth(to);
    ...
    asset previous_stake_amount = transfer ? s_itr->trans_quantity : s_itr->quantity;
    ...
    time_point t = current_time_point();
    microseconds
        curr_expiration_time = transfer ? s_itr->trans_expiration : s_itr->expiration,
    ...
    microseconds expiration_time = time_limit == 0 ? microseconds(0) : curr_time + microseconds(time_limit * MICROSEC_MULT);
    ...
    if (to_staked_balance && to != from)
    {
        ...
        if (transfer)
        {
            ...
            microseconds to_self_expiration = to_self_deleg == to_self_deleg_t.end() ? microseconds(0) : to_self_deleg->expiration;
        ...
        else
        {
            ...
            microseconds self_expiration = self_deleg == self_deleg_t.end() ? microseconds(0) : self_deleg->expiration;
...
ACTION boidtoken::syncwpf(const asset quantity)
{
    require_auth(get_self());
    config_t c_t(_self, _self.value);
    auto c_itr = c_t.find(0);
    ...
    bool transfer)
{
    ...
    asset curr_delegated_to = transfer ? to_itr->trans_quantity : to_itr->quantity;
...
extern "C"
{
    [[noreturn]] void apply(uint64_t receiver, uint64_t code, uint64_t action) {
        if (code == receiver)
        {
            switch (action)
            {
                EOSIO_DISPATCH_HELPER(boidtoken,
                                      (create)(issue)(recycle)
                                      //(reclaim)
                                      (open)(close)(transfer)(transtake)(stakebreak)(stake)(sendmessage)(claim)(unstake)(initstats)(erasestakes)(updatepower)(setpower)(matchtotdel)(synctotdel)(syncwpf)(setstakediff)(setpowerdiff)(setpowerrate)(setpwrstkmul)(setminstake)(setmaxpwrstk)(setmaxwpfpay)(setwpfproxy)(collectwpf)(recyclewpf)(setbpdecay)(setbpmult)(setbpconst)(resetbonus)(resetpowtm))
            }
        }
        eosio_exit(0);
    }
